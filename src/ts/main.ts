...
*/
const modalViews = document.querySelectorAll<HTMLElement>('.services-modal');
const modalBtns = document.querySelectorAll<HTMLElement>('.services-button');
const modalCloses = document.querySelectorAll<HTMLElement>('.services-modal-close');

// Ouvre le modal correspondant au bouton cliqué
// Opens the modal corresponding to the clicked button
const openModal = (modalIndex: number): void => {
...
if (workContainer) {
    ...
    const workFilters = document.querySelectorAll<HTMLElement>('.work-item');

    function activeWork(this: HTMLElement): void {
...
*/
const testimonialSwiperContainer = document.querySelector<HTMLElement>('.testimonial-container');
...
function scrollActive(): void {
    ...
    sections.forEach(current => {
        ...
        const navLink = document.querySelector<HTMLAnchorElement>(`.nav-menu a[href*=${sectionId}]`);
...
const getCurrentTheme = (): string => document.body.classList.contains(lightThemeClass) ? 'dark' : 'light';
const getCurrentIcon = (): string => themeButton?.classList.contains(iconTheme) ? 'bx-moon' : 'bx-sun';
